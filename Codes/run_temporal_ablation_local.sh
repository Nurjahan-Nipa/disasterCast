...
SEED=42
H3_EMBED_PATH="data/h3_embeddings_all.pkl"  # Path to graph embeddings

TOTAL_MODELS=$((${#CITIES[@]} * ${#HORIZONS[@]} * ${#ENCODERS[@]} * ${#MODES[@]}))

echo "=========================================="
echo "TEMPORAL ENCODER ABLATION - GRAPH H3 (LOCAL RUN)"
echo "=========================================="
...
echo "NOTE: Running locally may be slow for all configs."
echo ""
echo "=========================================="
echo ""
...
for city in "${CITIES[@]}"; do
    ...
    for horizon in "${HORIZONS[@]}"; do
        ...
        OUT_BASE="results/${city}_${horizon}_temporal_ablation_graph_h3"
        mkdir -p "$OUT_BASE"
        ...
        for encoder in "${ENCODERS[@]}"; do
            for mode in "${MODES[@]}"; do
                ...
                if [ "$mode" == "temp_h3" ]; then
                    USE_H3="--use-h3 --h3-embed-path $H3_EMBED_PATH"
        ...
        if metrics_path.exists():
            try:
                ...
                results.append({
                    ...
                    "Horizon": horizon,
                    "Encoder": encoder,
                    "H3": "Graph" if mode == "temp_h3" else "No",
                    "Model": model_name,
                    "MAE": round(m["MAE"], 4),
                    "RMSE": round(m["RMSE"], 4),
                    "R2": round(m["R2"], 4),
...
if results:
    ...
    try:
        shared_dir = Path("results/temporal_ablation_graph_h3")
...
results_dir = Path("results/temporal_ablation_graph_h3")
output_file = Path("results/temporal_ablation_graph_h3_final.csv")
...
echo ""
echo "========================================"
echo "ALL CITIES COMPLETE (LOCAL)"
echo "========================================"
